...
function SearchResults() {
  ...
  if (isSuccess && !data.videos?.length && !data.users?.length) {
    return (
      <NoResults title="No results found" text="Try different keywords or remove search filters" />
    );
  }
  ...
  return (
    <Wrapper>
      ...
      <StyledChannels>
        {isSuccess
          ? data.users.map((channel) => <ChannelInfo key={channel.id} channel={channel} />)
          : null}
      </StyledChannels>
