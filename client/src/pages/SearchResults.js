...
function SearchResults() {
  ...
  if (isSuccess && !data.videos?.length && !data.users?.length) {
    return <NoResults title="No results found" text="Try different keywords or remove search filters" />;
  }

  return (
    <Wrapper>
      <h2>Search Results</h2>
      <StyledChannels>{isSuccess ? data.users.map((channel) => <ChannelInfo key={channel.id} channel={channel} />) : null}</StyledChannels>
